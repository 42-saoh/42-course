...
/*   Updated: 2021/07/06 18:23:48 by saoh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			pickup_forks(t_ph *ph, int p_n)
{
	...
	c_time = get_time();
	write_state((int)(c_time - ph->p_d->first_time), ph, " has taken fork\n", 16);
}
...
static void			eating_philo(t_ph *ph)
{
	...
	c_time = get_time();
	ph->eat_flag = 1;
	write_state(c_time - ph->p_d->first_time, ph, " is eating\n", 11);
	eat_time = c_time + ph->p_d->t_t_e;
	while(c_time < eat_time)
	{
		usleep(20);
		c_time = get_time();
	}
...
static void			sleeping_philo(t_ph *ph)
{
	...
	c_time = get_time();
	write_state(c_time - ph->p_d->first_time, ph, " is sleeping\n", 13);
	...
	c_time = get_time();
	write_state(c_time - ph->p_d->first_time, ph, " is thinking\n", 13);
}
