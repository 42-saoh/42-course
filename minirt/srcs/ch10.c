...
/*   Updated: 2021/03/20 18:16:55 by saoh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list			*put_hitlst_plane(void)
{
	...
	hitlst = hitlst_new();
	p = init_plane(vec_create(0, 0, -100), vec_create(0, 0, 1), vec_create(0, 255, 255));
	hitlst_add(hitlst, (void *)p, OBJ_PLANE);
	p = init_plane(vec_create(-100, 0, 0.0), vec_create(1, 0, 0), vec_create(255, 0, 0));
	hitlst_add(hitlst, (void *)p, OBJ_PLANE);
	p = init_plane(vec_create(100, 0, 0.0), vec_create(-1, 0, 0), vec_create(0, 0, 255));
	hitlst_add(hitlst, (void *)p, OBJ_PLANE);
	p = init_plane(vec_create(0, 100, 0.0), vec_create(0, -1, 0), vec_create(255, 0, 255));
	hitlst_add(hitlst, (void *)p, OBJ_PLANE);
	p = init_plane(vec_create(0, 0, 100), vec_create(0, 0, -1), vec_create(255, 255, 0));
	hitlst_add(hitlst, (void *)p, OBJ_PLANE);
	cy = init_cylinder(vec_create(2, 0, -3.0), vec_create(0, 1, 0), 2, 2, vec_create(50, 150, 255));
	hitlst_add(hitlst, (void *)cy, OBJ_CYLINDER);
...
t_list			*put_lightlst(void)
{
	...
	l = init_light(vec_create(20, 0, 20.0), 0.1, vec_create(255, 128, 128));
	hitlst_add(l_lst, (void *)l, OBJ_LIGHT);
	l = init_light(vec_create(-20, 0, 20.0), 0.2, vec_create(255, 128, 128));
	hitlst_add(l_lst, (void *)l, OBJ_LIGHT);
	return (l_lst);
}
...
void			show_sphere_camera(int is_save)
{
	...
	cam = camera_locate_new(vec_create(0.0, 0.0, 0.0), vec_create(0, 0, -1),
