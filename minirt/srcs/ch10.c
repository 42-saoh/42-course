...
/*   Updated: 2021/03/18 17:10:25 by saoh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list			*put_hitlst_plane(void)
{
	...
	hitlst = hitlst_new();
	s = init_sphere(vec_create(0, 0, -1), 1);
	mat = mat_lambertian_new(vec_create(0.8, 0.8, 1));
	hitlst_mat_add(hitlst, (void *)s, OBJ_SPHERE, mat);
	free_material(mat);
	s = init_sphere(vec_create(0, -100.5, -1), 200);
	mat = mat_lambertian_new(vec_create(0.8, 0.8, 0));
	hitlst_mat_add(hitlst, (void *)s, OBJ_SPHERE, mat);
	free_material(mat);
	return (hitlst);
